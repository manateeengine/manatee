...
pub fn init(allocator: std.mem.Allocator, config: system.Window.WindowConfig) !Self {
...
fn getDimensions(ctx: *anyopaque) system.Window.Dimensions {
    const self: *Self = @ptrCast(@alignCast(ctx));
    const window_rect = self.native_window.getWindowRect() catch win32.display_devices.Rect{
        .bottom = 0,
        .left = 0,
        .right = 0,
        .top = 0,
    };
    return system.Window.Dimensions{
...
fn wndProc(current_window: *win32.wnd_msg.Window, msg: u32, w_param: usize, l_param: isize) callconv(std.builtin.CallingConvention.winapi) isize {
    return switch (msg) {
